...
mocha_1.describe('Compat', function () {
    mocha_1.describe('jobs getters', function () {
        ...
        mocha_1.beforeEach(async function () {
            ...
            queue = new compat_1.Queue3(queueName);
        });
        ...
        mocha_1.it('should get completed jobs', function (done) {
        ...
        mocha_1.it('should get failed jobs', function (done) {
            queue.process(async (job) => {
        ...
        mocha_1.it('should return all completed jobs when not setting start/end', function (done) {
            queue.process(async (job) => { });
            queue.on('completed', lodash_1.after(3, async function () {
                try {
        ...
        mocha_1.it('should return all failed jobs when not setting start/end', function (done) {
            ...
            queue.on('failed', lodash_1.after(3, async function () {
                try {
                    queue;
        ...
        mocha_1.it('should return jobs for multiple types', function (done) {
            ...
            queue.on('completed', lodash_1.after(2, async function () {
                try {
    ...
    mocha_1.describe('events', function () {
        ...
        mocha_1.it('should emit global waiting event when a job has been added', function (done) {
            queue.on('waiting', function () {
                done();
            });
            queue.add('test', { foo: 'bar' });
        ...
        mocha_1.it('should emit an event when a job becomes active', function (done) {
            queue.add('test', {});
            queue.process(async () => { });
    ...
    mocha_1.describe('Pause', function () {
        ...
        mocha_1.it('should pause the queue locally', async () => {
            ...
            const processPromise = new Promise(resolve => {
                process = async (job) => {
                    chai_1.expect(queue.isWorkerPaused()).to.be.eql(false);
                    counter--;
                    if (counter === 0) {
                        resolve();
                    }
            ...
            chai_1.expect(queue.isWorkerPaused()).to.be.eql(true);
        ...
        mocha_1.it('should pause the queue locally when more than one worker is active', async () => {
            ...
            const worker1 = new classes_1.Worker(queueName, process1);
            await worker1.waitUntilReady();
            const worker2 = new classes_1.Worker(queueName, process2);
...
//# sourceMappingURL=test_compat.js.map